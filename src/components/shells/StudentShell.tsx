...
const NAV_GROUPS: SidebarGroup[] = [
  ...
  },
  {
    label: 'Account',
    items: [
      { page: 'settings', label: 'Settings', icon: <Settings size={ICON} aria-hidden /> },
  ...
  },
  {
    label: 'Practice',
    items: [
      { page: 'practice-studio', label: 'Practice Studio', icon: <FlaskConical size={ICON} aria-hidden /> },
      ...
      { page: 'practice-powerbi', label: 'Power BI', icon: <BarChart3 size={ICON} aria-hidden /> },
            ...
            {group.items.map((item) => (
              <SidebarLink
                ...
                active={
                  ...
                  (item.page === 'assignments' && currentPage === 'quiz')
                }
...
function NavToggleButton({
  ...
  return (
    <button
      ...
      onClick={onToggle}
      className={cn(
        'inline-flex items-center gap-1.5 rounded-lg border border-[#E5E7EB] bg-white px-2.5 py-1.5 text-[12px] font-semibold text-[#0A1020] shadow-sm transition-colors hover:bg-zinc-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2563EB]/40',
        className,
      )}
...
export function StudentShell({ currentPage, user, onNavigate, onLogout, children }: StudentShellProps) {
  ...
  const isWorkbench =
    currentPage === 'practice-code' ||
    currentPage === 'practice-sql' ||
    currentPage === 'practice-typing' ||
    currentPage === 'practice-powerbi'
  const canvasBg = isWorkbench ? 'bg-[#0B1020]' : 'bg-[#F4F5F7]'
  return (
    <div className={cn('flex h-dvh max-h-dvh min-h-0 overflow-hidden', canvasBg)}>
      <aside
        className={cn(
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 w-[min(18rem,88vw)] max-w-full bg-[#0A1020] transition-transform duration-200 lg:hidden',
      ...
      >
        <button
          type="button"
          onClick={() => setMobileOpen(false)}
          className="absolute right-2 top-3 grid h-11 w-11 place-items-center rounded-lg text-white/60 outline-none hover:bg-white/10 hover:text-white focus-visible:ring-2 focus-visible:ring-blue-400/50"
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        <div
          className={cn(
            'flex h-12 flex-shrink-0 items-center gap-2 border-b px-3 lg:h-11',
            isWorkbench
              ? 'border-[#26324A] bg-[#0F172A]'
        ...
        >
          <button
            ...
            className={cn(
              'grid h-11 w-11 place-items-center rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-blue-400/50 lg:hidden',
              isWorkbench
                ? 'text-[#E5E7EB] hover:bg-white/10'
                : 'text-[#0A1020] hover:bg-[#0A1020]/5',
            )}
          ...
          <NavToggleButton
            ...
            className={cn(
              ...
              isWorkbench &&
                'border-[#26324A] bg-[#111827] text-[#E5E7EB] hover:bg-[#1a2332]',
            )}
          />
          ...
          >
            ...
            <span
              className={cn(
                ...
                isWorkbench ? 'text-[#E5E7EB]' : 'text-[#0A1020]',
              )}
            >
              CodeQuest
            </span>
        ...
        <main
          ...
          className={cn(
            ...
            isWorkbench || currentPage === 'progress'
              ? 'overflow-hidden'
