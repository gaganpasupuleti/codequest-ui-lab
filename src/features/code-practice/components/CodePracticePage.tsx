...
export function CodePracticePage() {
  ...
  const questionPicker = languageQuestions.length > 0 ? (
    <div className="flex flex-wrap items-center gap-1.5 border-b border-[#26324A] bg-[#0F172A] px-3 py-1.5">
      <span className="text-[11px] font-semibold uppercase tracking-wider text-[#94A3B8]">Question</span>
  ...
  return (
    <CodeWorkbenchLayout
      ...
      livePreview={
        language === 'react' ? (
          <LivePreviewPanel language={language} code={code} questionTitle={question?.title} />
        ) : undefined
      }
