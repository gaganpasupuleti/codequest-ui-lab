...
import { PracticeStudioPage } from '@/components/pages/PracticeStudioPage'
...
function App() {
  ...
  useEffect(() => {
    ...
    void fetchCurrentUser(token).then((currentUser) => {
      if (cancelled) return
      if (!currentUser) {
        // Keep UI-lab / demo fake sessions when backend /me is unavailable.
  ...
  return (
    <div className={wrapperClass}>
      ...
      >
        ...
        {studentPage === 'live-classes' && <LiveClassesPage user={user} />}

        {studentPage === 'practice-studio' && (
          <PracticeStudioPage onNavigate={handleStudentNavigate} />
        )}

        {studentPage === 'assignments' && (
          <AssignmentsPage
            onNavigate={handleStudentNavigate}
            onBeforeSelectQuiz={handleBeforeSelectQuiz}
          />
        )}
